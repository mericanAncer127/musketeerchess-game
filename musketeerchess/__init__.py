...
BB_KNIGHT_SIDE_ATTACKS = [_step_attacks(sq, [10, 6, -10, -6]) for sq in SQUARES]
...
class BaseBoard:
    ...
    def _clear_board(self) -> None:
        ...
        self.unicorns = BB_EMPTY
        self.dragons = BB_EMPTY
        self.chancellors = BB_EMPTY
        self.archbishops = BB_EMPTY
        self.elephants = BB_EMPTY
        self.hawks = BB_EMPTY
        self.fortresses = BB_EMPTY
        self.spiders = BB_EMPTY
    ...
    def attacks_mask(self, square: Square) -> Bitboard:
        ...
        if bb_square & self.pawns:
            ...
            return BB_KNIGHT_SIDE_ATTACKS[square] | BB_KING_ATTACKS[square] | BB_HOR_VER_2_ATTACKS[square][BB_HOR_VER_2_MASKS[square] & self.occupied]
    ...
    def apply_transform(self, f: Callable[[Bitboard], Bitboard]) -> None:
        ...
        self.kings = f(self.queens)
        self.leopards = f(self.leopards)
        self.cannons = f(self.cannons)
        self.unicorns = f(self.unicorns)
        self.dragons = f(self.dragons)
        self.chancellors = f(self.chancellors)
        self.archbishops = f(self.archbishops)
        self.elephants = f(self.elephants)
        self.hawks = f(self.hawks)
        self.fortresses = f(self.fortresses)
        self.spiders = f(self.spiders)
    ...
    def copy(self: BaseBoardT) -> BaseBoardT:
        ...
        board.unicorns = self.unicorns
        board.dragons = self.dragons
        board.chancellors = self.chancellors
        board.archbishops = self.archbishops
        board.elephants = self.elephants
        board.hawks = self.hawks
        board.fortresses = self.fortresses
        board.spiders = self.spiders
...
class _BoardState(Generic[BoardT]):
    ...
    def __init__(self, board: BoardT) -> None:
        ...
        self.unicorns = board.unicorns
        self.dragons = board.dragons
        self.chancellors = board.chancellors
        self.archbishops = board.archbishops
        self.elephants = board.elephants
        self.hawks = board.hawks
        self.fortresses = board.fortresses
        self.spiders = board.spiders
...
class Board(BaseBoard):
    ...
    def __init__(self: BoardT, fen: Optional[str] = STARTING_FEN, *, chess960: bool = False) -> None:
        ...
        """
        black and white selects each's musketeer piece and share them.
        whitew: White's piece that white selected
        whiteb: White's piece that black selected
        blackw: Black's piece that white selected
        blackb: Black's piece that black selected
        """

        self.musketeer_pieces = {'white': None, 'black': None}
        self.musketeer_columns = {'whitew': None, 'whiteb': None, 'blackw': None, 'blackb': None}
        self.musketeer_unlaunched = {'whitew': True, 'whiteb': True, 'blackw': True, 'blackb': True}
    ...
    def select_black_musteer_piece(self, piece: str) -> None:
        """Selects the Musketeer pieces for Black player.
        ...
        """
        if piece.__len__() > 1 :
            piece = PIECE_SYMBOLS[PIECE_NAMES.index(piece)]
        if piece is not None:
            self.musketeer_pieces['black'] = PIECE_SYMBOLS.index(piece)

    def select_white_musteer_piece(self, piece: str) -> None:
        """Selects the Musketeer pieces for Black player.
        set musketeer piece nams as 1 letter
        piece name sould be musketeer piece name:
        ...
        if piece.__len__() > 1 :
            piece = PIECE_SYMBOLS[PIECE_NAMES.index(piece)]
        if piece is not None:
            self.musketeer_pieces['white'] = PIECE_SYMBOLS.index(piece)

    def place_musketeer_piece(self, item: str, column: str) -> None:
        """Declares the column for the Musketeer piece and updates the board.
        item could be following:
        ...
        """
        self.musketeer_columns[item] = FILE_NAMES.index(column)
        # Update the board with the Musketeer piece placement
        pass

    def is_all_musketeer_placed(self) -> bool :
        """
        Tells if all of the musketeer pieces were placed basically.

        """
        return self.musketeer_columns['whitew'] and self.musketeer_columns['whiteb'] and self.musketeer_columns['blackw'] and self.musketeer_columns['blackb']

    def can_launch_musketeer(self, name: str) -> bool :
        result = False
        sq = self.musketeer_columns[name]
        if name.startswith('black'): sq += 56
        piece = self.piece_at(sq)
        if self.musketeer_unlaunched[name]:
            return piece == None
        return False
    def unlaunch_musketeer(self, move : Move) -> None:
        sq = Square(move.from_square)
        piece = self.piece_at(sq)
        if not piece:
            return
        piece = PIECE_SYMBOLS.index(piece.symbol().lower())
        if move.from_square == self.musketeer_columns['whitew'] and piece == self.musketeer_pieces['white']:
            self.musketeer_unlaunched['whitew'] = True
        if move.from_square == self.musketeer_columns['whiteb'] and piece == self.musketeer_pieces['black']:
            self.musketeer_unlaunched['whiteb'] = True
        if move.from_square == self.musketeer_columns['blackw'] + 56 and piece == self.musketeer_pieces['white']:
            self.musketeer_unlaunched['blackw'] = True
        if move.from_square == self.musketeer_columns['blackb'] + 56 and piece == self.musketeer_pieces['black']:
            self.musketeer_unlaunched['blackb'] = True
    ...
    def launch_musketeer(self) -> None:
        ...
        if self.can_launch_musketeer('whitew') and self.musketeer_pieces['white'] and self.turn is BLACK :
            piece = Piece(self.musketeer_pieces['white'], WHITE)
            square = Square(self.musketeer_columns['whitew'])
            self.place_piece_at(square, piece)
            self.musketeer_unlaunched['whitew'] = False
            return

        if self.can_launch_musketeer('whiteb') and self.musketeer_pieces['black'] and self.turn is BLACK :
            piece = Piece(self.musketeer_pieces['black'], WHITE)
            square = Square(self.musketeer_columns['whiteb'])
            self.place_piece_at(square, piece)
            self.musketeer_unlaunched['whiteb'] = False
            return

        if self.can_launch_musketeer('blackw') and self.musketeer_pieces['white'] and self.turn is WHITE :
            piece = Piece(self.musketeer_pieces['white'], BLACK)
            square = Square(8 * 7 + self.musketeer_columns['blackw'])
            self.place_piece_at(square, piece)
            self.musketeer_unlaunched['blackw'] = False
            return
                
        if self.can_launch_musketeer('blackb') and self.musketeer_pieces['black'] and self.turn is WHITE :
            piece = Piece(self.musketeer_pieces['black'], BLACK)
            square = Square(8 * 7 + self.musketeer_columns['blackb'])
            self.place_piece_at(square, piece)
            self.musketeer_unlaunched['blackb'] = False
            return
        pass
    ...
    def place_piece_at(self, square: Square, piece: Optional[Piece], promoted: bool = False) -> None:
        super().set_piece_at(square, piece, promoted=promoted)
        # self.clear_stack()
    ...
    def __str__(self) -> str:
        ...
        if self.musketeer_pieces['white'] and self.musketeer_pieces['black']:
            for square in range(0, 8):
                if self.musketeer_unlaunched['blackw'] and square is self.musketeer_columns['blackw']:
                    builder.append(Piece(self.musketeer_pieces['white'], BLACK).symbol())
                elif self.musketeer_unlaunched['blackb'] and square is self.musketeer_columns['blackb']:
                    builder.append(Piece(self.musketeer_pieces['black'], BLACK).symbol())
                else:
                    builder.append(".")
                builder.append(" ")
            builder.append("\n")
        ...
        if self.musketeer_pieces['white'] and self.musketeer_pieces['black']:
            for square in range(0, 8):
                if self.musketeer_unlaunched['whitew'] and square is self.musketeer_columns['whitew']:
                    builder.append(Piece(self.musketeer_pieces['white'], WHITE).symbol())
                elif self.musketeer_unlaunched['whiteb'] and square is self.musketeer_columns['whiteb']:
                    builder.append(Piece(self.musketeer_pieces['black'], WHITE).symbol())
                else:
                    builder.append(".")
                builder.append(" ")
