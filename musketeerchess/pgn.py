...
import musketeerchess as chess
import musketeerchess.engine
import musketeerchess.svg
...
from musketeerchess import Color, Square
...
MOVETEXT_REGEX = re.compile(r"""
    (
        [NBKRQLCUDMAEHFS]?[a-h]?[1-8]?[\-x]?[a-h][1-8](?:=?[nbrqkNBRQKLCUDMAEHFS])?
        |[PNBRQKLCUDMAEHFS]?@[a-h][1-8]
        |--
        |Z0
        |0000
        |@@@@
...
class GameNode(abc.ABC):
    ...
    def mainline_moves_musketeer(self, white, black, whitew, whiteb, blackw, blackb) -> Mainline[chess.Move]:
        """Returns an iterable over the main moves after this node."""
        return Mainline(self, lambda node: node.move, white, black, whitew, whiteb, blackw, blackb)
...
class Game(GameNode):
    ...
    def select_and_place_musketeer_pieces(self, white: str, black: str, whitew: str, whiteb: str, blackw: str, blackb: str):
        board = self.board()
    ...
    def accept_musketeer(self, visitor: BaseVisitor[ResultT], white: str, black: str, whitew: str, whiteb: str, blackw: str, blackb: str) -> ResultT:
        """
        Traverses the game in PGN order using the given *visitor*. Returns
        the *visitor* result.
        """
...
class Mainline(Generic[MainlineMapT]):
    def __init__(self, start: GameNode, f: Callable[[ChildNode], MainlineMapT], white = None, black = None, whitew = None, whiteb = None, blackw = None, blackb = None) -> None:
        self.start = start
        self.f = f
    ...
    def process_musketeer(self, board : musketeerchess.Board):
        board.select_white_musteer_piece(self.white)
        board.select_black_musteer_piece(self.black)

        board.place_musketeer_piece('whitew', self.whitew)
        board.place_musketeer_piece('whiteb', self.whiteb)
        board.place_musketeer_piece('blackw', self.blackw)
        board.place_musketeer_piece('blackb', self.blackb)
...
def read_game(handle: TextIO, *, Visitor: Any = GameBuilder) -> Any:
    ...
    if not skipping_game:
        ...
        try:
            ...
            white = headers.get("MusketeerWhite")
            black = headers.get("MusketeerBlack")

            whitew = headers.get("MusketeerPosWW")
            whiteb = headers.get("MusketeerPosWB")
            blackw = headers.get("MusketeerPosBW")
            blackb = headers.get("MusketeerPosBB")
