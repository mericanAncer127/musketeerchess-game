...
def draw_panel():
   ...
   text = font.render('Musketeer Chess', True, black)
...
def read_from_pgn(file_path):
   ...
   ogame = pgn.read_game(pgnfile)
   white = ogame.headers['MusketeerWhite']
   black = ogame.headers['MusketeerBlack']

   whitew = ogame.headers['MusketeerPosWW']
   whiteb = ogame.headers['MusketeerPosWB']
   blackw = ogame.headers['MusketeerPosBW']
   blackb = ogame.headers['MusketeerPosBB']
   ...
   # Go through each move in the ogame until 
   # we reach the required move number
   moveArray = ogame.mainline_moves_musketeer(white, black, whitew, whiteb, blackw, blackb)
   for number, move in enumerate(moveArray): 
      # It copies the move played by each  
      # player on the virtual board 
      board.push(move) 
      node = node.add_variation(musketeerchess.Move.from_uci(move.uci()))
